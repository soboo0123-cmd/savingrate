...
import requests # 파일 최상단에 이 줄이 추가되어야 합니다. (smtplib, email.mime 등은 삭제)
...
def send_change_email(old_data, new_data):
    ...
    # 본인의 Mailgun Sandbox 도메인 주소로 변경하세요.
    MAILGUN_DOMAIN = "sandbox6c6629c855304d2b8f19ce3156922005.mailgun.org"
    ...
    try:
        response = requests.post(
            ...
            auth=("api", api_key),
            verify=False,
            data={"from": f"우체국 금리알림 <postmaster@{MAILGUN_DOMAIN}>",
                  "to": [recipient],
                  "subject": subject,
                  "html": html_content} # 'text' 대신 'html'을 쓰면 표가 예쁘게 전송됩니다.
        )
        
        # 발송 성공 여부 확인
        if response.status_code == 200:
            print("✅ Mailgun API를 통한 변동 알림 메일 발송 성공!")
        else:
            print(f"❌ Mailgun 발송 실패: {response.status_code} - {response.text}")
