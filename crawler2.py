...
def send_change_email(old_data, new_data):
    # 1. 깃허브 환경변수(Secrets)에서 메일 정보 가져오기
    smtp_username = os.environ.get('MAIL_ID')       # 본인 지메일 주소
    smtp_password = os.environ.get('MAIL_PASSWORD') # 지메일 앱 비밀번호
    recipient = os.environ.get('RECIPIENT_EMAIL')   # 받을 사람 주소 (본인)

    # 환경변수가 없으면 실행 취소 (로컬 테스트 에러 방지)
    ...
    try:
        with smtplib.SMTP('smtp-relay.brevo.com', 587) as server: # <--- 이렇게 변경
            ...
            print("✅ Brevo를 통한 변동 알림 메일 발송 성공!")
